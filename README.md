...
In particular:
- Script `nonparametric.R` develops the code needed to model both the short- and long-term dynamics of train delays without considering any covariates. It includes the estimation of the transition intensities between delay states, cumulative hazards, and transition probabilities of moving from an initial state to another over the time interval [0,30min];
- Script `semiparametric.R` follows the same steps of the prevoius one, but focusing on the effects of factors such as: boarding and alighting passengers, weather conditions, and train frequency at each station.
