...
In particular:
- Script `preprocessing.R` reports data preprocessing;
- Scripts `NP_3.Rmd` and `NP_4.Rmd` develop the code needed to model both the short- and long-term dynamics of train delays without considering any covariates, respectively for the 3-state and the 4-state models. They include the estimation of the transition intensities between delay states, cumulative hazards, and transition probabilities of moving from an initial state to another over the time interval [0,30min];
- Script `semi.Rmd` follows the same steps of the prevoius ones, but focusing on the effects of factors such as: boarding and alighting passengers, weather conditions, and train frequency at each station. It also contains the code to perform frailty models.
